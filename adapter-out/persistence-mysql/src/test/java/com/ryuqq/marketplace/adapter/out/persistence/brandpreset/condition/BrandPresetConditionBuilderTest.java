...
/**
 * BrandPresetConditionBuilderTest - BrandPreset 조건 빌더 단위 테스트.
 *
...
@DisplayName("BrandPresetConditionBuilder 단위 테스트")
class BrandPresetConditionBuilderTest {

    private BrandPresetConditionBuilder conditionBuilder;

    @Mock private BrandPresetSearchCriteria criteria;

    @BeforeEach
    void setUp() {
        conditionBuilder = new BrandPresetConditionBuilder();
    }
    ...
    class SearchConditionTest {
        ...
        @Test
        @DisplayName("BRAND_NAME 필드로 검색 시 BooleanExpression을 반환합니다")
        void searchCondition_WithBrandNameField_ReturnsBooleanExpression() {
            // given
            given(criteria.hasSearchFilter()).willReturn(true);
            given(criteria.searchField()).willReturn("BRAND_NAME");
            given(criteria.searchWord()).willReturn("브랜드");
        ...
        @Test
        @DisplayName("BRAND_CODE 필드로 검색 시 BooleanExpression을 반환합니다")
        void searchCondition_WithBrandCodeField_ReturnsBooleanExpression() {
            // given
            given(criteria.hasSearchFilter()).willReturn(true);
            given(criteria.searchField()).willReturn("BRAND_CODE");
            given(criteria.searchWord()).willReturn("B123");
