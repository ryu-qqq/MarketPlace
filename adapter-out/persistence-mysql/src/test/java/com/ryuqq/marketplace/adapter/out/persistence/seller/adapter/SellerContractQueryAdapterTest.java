...
import com.ryuqq.marketplace.adapter.out.persistence.seller.repository.SellerContractQueryDslRepository;
...
/**
 * SellerContractQueryAdapterTest - 셀러 계약 정보 Query Adapter 단위 테스트.
 *
...
@DisplayName("SellerContractQueryAdapter 단위 테스트")
class SellerContractQueryAdapterTest {

    @Mock private SellerContractQueryDslRepository queryDslRepository;

    @Mock private SellerContractJpaEntityMapper mapper;

    @InjectMocks private SellerContractQueryAdapter queryAdapter;
    ...
    class FindByIdTest {
        ...
        @Test
        @DisplayName("ID로 계약 정보를 조회하여 Domain으로 변환합니다")
        void findById_WithValidId_ReturnsDomain() {
            // given
            SellerContractId id = SellerContractId.of(1L);
            ...
            // when
            Optional<SellerContract> result = queryAdapter.findById(id);
        ...
        void findById_WithNonExistentId_ReturnsEmpty() {
            // given
            SellerContractId id = SellerContractId.of(999L);
    ...
    class FindBySellerIdTest {
        ...
        @Test
        @DisplayName("셀러 ID로 계약 정보를 조회하여 Domain으로 변환합니다")
        void findBySellerId_WithValidSellerId_ReturnsDomain() {
            ...
            // when
            Optional<SellerContract> result = queryAdapter.findBySellerId(sellerId);
    ...
    class ExistsBySellerIdTest {
        ...
        @Test
        @DisplayName("셀러 ID로 계약 정보가 존재하면 true를 반환합니다")
        ...
        @Test
        @DisplayName("셀러 ID로 계약 정보가 존재하지 않으면 false를 반환합니다")
