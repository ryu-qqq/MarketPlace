package com.company.marketplace.adapter.out.persistence.architecture;
 ...
 *
 * <p>Persistence Layer의 데이터 접근 패턴 일관성을 검증합니다:</p>
 *
 * <p><strong>검증 규칙:</strong></p>
 * <ul>
 *   <li>규칙 1: QueryDslRepository는 JPAQueryFactory 필드 필수</li>
 *   <li>규칙 2: QueryDslRepository는 QType static final 필드 필수</li>
 *   <li>규칙 3: QueryAdapter는 QueryDslRepository 의존 필수</li>
 *   <li>규칙 4: CommandAdapter는 JpaRepository 의존 필수</li>
 *   <li>규칙 5: QueryDslRepository는 DTO Projection 사용 (Entity 반환 금지)</li>
 *   <li>규칙 6: Repository는 Domain 반환 금지 (DTO만 반환)</li>
 *   <li>규칙 7: Test Fixtures는 fixture() 메서드 패턴 사용</li>
 *   <li>규칙 8: Test Fixtures는 Builder 패턴 금지</li>
 * </ul>
 *
 * <p><strong>참고:</strong></p>
 * <ul>
 *   <li>N+1 문제 예방: Join 사용은 수동 코드 리뷰로 검증 필요</li>
 *   <li>DTO Projection: Projections.constructor() 사용 권장</li>
 * </ul>
 *
...
class DataAccessPatternArchTest {
    ...
    void queryDslRepository_MustHaveJPAQueryFactoryField() {
        ArchRule rule = classes()
            ...
            .should().dependOnClassesThat().haveSimpleName("JPAQueryFactory")
    ...
    void queryDslRepository_MustHaveQTypeStaticField() {
        ArchRule rule = fields()
            ...
            .and().areStatic()
            .and().areFinal()
            .should().haveRawType(com.tngtech.archunit.base.DescribedPredicate.describe(
                "Q-type class",
    ...
    void queryDslRepository_ShouldNotReturnJpaEntity() {
        ArchRule rule = noMethods()
            ...
            .should().haveRawReturnType(com.tngtech.archunit.base.DescribedPredicate.describe(
                "JPA Entity class",
                javaClass -> javaClass.getSimpleName().endsWith("JpaEntity")
            ))
            .because("QueryDslRepository는 JpaEntity 대신 DTO Projection을 사용해야 합니다 (N+1 예방)");
    ...
    void repository_MustNotReturnDomain() {
        ArchRule rule = noMethods()
            .that().areDeclaredInClassesThat().haveSimpleNameContaining("Repository")
    ...
    void testFixtures_ShouldProvideFixtureMethod() {
        ArchRule rule = methods()
            .that().areDeclaredInClassesThat().haveSimpleNameEndingWith("Fixture")
            .and().arePublic()
            .and().areStatic()
            .should().haveNameMatching(".*fixture.*")
    ...
    void testFixtures_MustNotUseBuilderPattern() {
        ArchRule rule = noClasses()
            ...
            .should().dependOnClassesThat().haveSimpleNameContaining("Builder")
    ...
    void queryDslRepository_MustOnlyHaveStandardMethods() {
        ArchRule rule = methods()
            ...
            .because("QueryDslRepository는 4개 표준 메서드만 허용합니다 (findById, existsById, findByCriteria, countByCriteria)");
    ...
    void adapter_MustNotUseJPAQueryFactoryDirectly() {
        ArchRule rule = noClasses()
            .that().haveSimpleNameEndingWith("Adapter")
            .should().dependOnClassesThat().haveSimpleName("JPAQueryFactory")
            .because("Adapter는 JPAQueryFactory를 직접 사용하면 안 됩니다 (QueryDslRepository를 통해서만 접근)");
