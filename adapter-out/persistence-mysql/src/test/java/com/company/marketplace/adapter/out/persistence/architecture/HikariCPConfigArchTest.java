 ...
 *
 * <p>HikariCP Connection Pool 설정 관련 아키텍처 규칙을 검증합니다:</p>
 *
 * <p><strong>검증 규칙:</strong></p>
 * <ul>
 *   <li>규칙 1: DataSourceConfig는 @Configuration 필수</li>
 *   <li>규칙 2: Config 클래스는 config 패키지에 위치</li>
 *   <li>규칙 3: Config 클래스는 public이어야 함</li>
 *   <li>규칙 4: Config 클래스는 final 금지 (프록시 생성)</li>
 *   <li>규칙 5: Config는 Entity/Repository 의존 금지</li>
 *   <li>규칙 6: Config는 Domain/Application Layer 의존 금지</li>
 *   <li>규칙 7: DataSource 설정 클래스는 단일 책임</li>
 * </ul>
 *
 * <p><strong>필수 설정 (application.yml에서 검증):</strong></p>
 * <ul>
 *   <li>OSIV 비활성화: spring.jpa.open-in-view: false</li>
 *   <li>DDL Auto: spring.jpa.hibernate.ddl-auto: validate</li>
 *   <li>Pool Size: spring.datasource.hikari.maximum-pool-size: 20</li>
 *   <li>Connection Timeout: spring.datasource.hikari.connection-timeout: 30000</li>
 *   <li>Max Lifetime: spring.datasource.hikari.max-lifetime: 1800000</li>
 * </ul>
 *
 * <p><strong>참고:</strong></p>
 * <ul>
 *   <li>application.yml 설정 검증은 통합 테스트 필요</li>
 *   <li>ArchUnit은 클래스 기반 검증만 가능</li>
 *   <li>Runtime 설정 검증은 별도 테스트 권장</li>
 * </ul>
 *
...
class HikariCPConfigArchTest {
    ...
    void dataSourceConfig_MustHaveConfigurationAnnotation() {
        ArchRule rule = classes()
            .that().haveSimpleNameContaining("DataSourceConfig")
            .or().haveSimpleNameContaining("HikariConfig")
    ...
    void dataSourceConfig_MustNotDependOnDomainOrApplication() {
        ArchRule rule = noClasses()
            ...
            .because("DataSourceConfig는 Infrastructure Layer 설정으로 Domain/Application을 의존하면 안 됩니다");
    ...
    void dataSourceConfig_ShouldOnlyConfigureDataSource() {
        ArchRule rule = classes()
            ...
            .because("DataSourceConfig는 DataSource/HikariCP와 Spring Framework 관련 클래스만 의존해야 합니다");
    ...
    void dataSourceConfig_ShouldBeSeparateFromJpaConfig() {
        ArchRule rule = noClasses()
            .that().haveSimpleNameContaining("DataSourceConfig")
            .should().haveSimpleNameContaining("Jpa")
    ...
    void config_MustNotDependOnAdapterOrMapper() {
        ArchRule rule = noClasses()
            ...
            .should().dependOnClassesThat().resideInAnyPackage("..adapter..", "..mapper..")
