...
public class CancelQueryAdapter implements CancelQueryPort {
    ...
    public List<Cancel> findAllByOrderItemId(OrderItemId orderItemId) {
        return cancelRepository.findAllByOrderItemId(orderItemId.value()).stream()
                .map(mapper::toDomain)
                .toList();
    }
    ...
    public List<Cancel> findByCriteria(CancelSearchCriteria criteria) {
        return cancelRepository.findByCriteria(criteria).stream().map(mapper::toDomain).toList();
    }
