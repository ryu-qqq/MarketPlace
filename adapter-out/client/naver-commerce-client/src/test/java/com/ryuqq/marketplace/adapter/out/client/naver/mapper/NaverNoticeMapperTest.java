...
class NaverNoticeMapperTest {
    ...
    private NoticeCategoryResult categoryWithFields(String code, List<NoticeFieldResult> fields) {
        return new NoticeCategoryResult(
                1L, code, "카테고리명", "Category", "CLOTHING", true, fields, Instant.now());
    }
    ...
    class MapNoticeTest {
        ...
        void noCategoryDefaultsToEtc() {
            var entries = List.of(new ProductNoticeEntryResult(1L, 100L, "테스트값"));
        ...
        void clothingMapsToWear() {
            var fields = List.of(new NoticeFieldResult(100L, "material", "소재", true, 1));
            var category = categoryWithFields("CLOTHING", fields);
            var entries = List.of(new ProductNoticeEntryResult(1L, 100L, "면 100%"));
            var notice = noticeWithEntries(entries);
        ...
        void shoesMapsToShoes() {
            var fields = List.of(new NoticeFieldResult(100L, "material_upper", "갑피", true, 1));
            var category = categoryWithFields("SHOES", fields);
            var entries = List.of(new ProductNoticeEntryResult(1L, 100L, "가죽"));
            var notice = noticeWithEntries(entries);
            var syncData = createSyncDataWithNotice(notice, category);
        ...
        void bagsMapsToBAG() {
            var fields = List.of(new NoticeFieldResult(100L, "type", "종류", true, 1));
            var category = categoryWithFields("BAGS", fields);
            var entries = List.of(new ProductNoticeEntryResult(1L, 100L, "백팩"));
            var notice = noticeWithEntries(entries);
            var syncData = createSyncDataWithNotice(notice, category);
        ...
        void accessoriesMapsToFashionItems() {
            ...
            var entries = List.of(new ProductNoticeEntryResult(1L, 100L, "벨트"));
            var notice = noticeWithEntries(entries);
            var syncData = createSyncDataWithNotice(notice, category);
    ...
    class MapOriginAreaInfoTest {
        ...
        void madeInFieldUsed() {
            var fields = List.of(new NoticeFieldResult(100L, "made_in", "제조국", true, 1));
            var category = categoryWithFields("CLOTHING", fields);
            var entries = List.of(new ProductNoticeEntryResult(1L, 100L, "이탈리아"));
            var notice = noticeWithEntries(entries);
            var syncData = createSyncDataWithNotice(notice, category);
        ...
        void blankMadeInReturnsDefault() {
            var fields = List.of(new NoticeFieldResult(100L, "made_in", "제조국", true, 1));
            var category = categoryWithFields("CLOTHING", fields);
            var entries = List.of(new ProductNoticeEntryResult(1L, 100L, "  "));
            var notice = noticeWithEntries(entries);
            var syncData = createSyncDataWithNotice(notice, category);
