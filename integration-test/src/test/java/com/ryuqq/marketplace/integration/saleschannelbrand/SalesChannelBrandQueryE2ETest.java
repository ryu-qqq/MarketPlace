package com.ryuqq.marketplace.integration.saleschannelbrand;
...
/**
 * Sales Channel Brand Query 엔드포인트 E2E 테스트.
 *
 * <p>테스트 대상: - GET /sales-channels/{salesChannelId}/brands - 판매채널 브랜드 목록 조회
 *
 ...
 */
@Tag("e2e")
@Tag("saleschannelbrand")
@Tag("query")
@DisplayName("Sales Channel Brand Query API E2E 테스트")
class SalesChannelBrandQueryE2ETest extends E2ETestBase {

    private static final String BASE_URL = "/sales-channels/{salesChannelId}/brands";
    ...
    // ===== GET /sales-channels/{salesChannelId}/brands - 판매채널 브랜드 목록 조회 =====

    @Nested
    @DisplayName("GET /sales-channels/{salesChannelId}/brands - 판매채널 브랜드 목록 조회")
    class SearchBrandsTest {

        @Test
        @Tag("P0")
        @DisplayName("[Q1-1] 데이터 존재 시 정상 조회")
        void searchBrands_withData_returnsOk() {
            // given: 판매채널 생성 후 브랜드 3건 등록
            var channel =
        ...
        void searchBrands_nonExistingChannel_returns404OrEmpty() {
        ...
        void searchBrands_authenticatedUser_returns403() {
            // given
            var channel =
                    salesChannelRepository.save(
                            SalesChannelJpaEntityFixtures.activeEntityWithName("쿠팡"));
    ...
    class FullFlowTest {
        ...
        @Test
        @Tag("P0")
        @DisplayName("[F1] 판매채널 생성 → 브랜드 등록 → 조회 플로우 (Query Only)")
        void fullFlow_channelToListBrands() {
            // Step 1: 판매채널 생성
            var channel =
                    salesChannelRepository.save(
                            SalesChannelJpaEntityFixtures.activeEntityWithName("쿠팡"));
            ...
            // DB 검증
            assertThat(salesChannelBrandRepository.count()).isEqualTo(2);
        }
    }
}
