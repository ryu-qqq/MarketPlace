...
/**
 * QnA Query Testcontainers E2E 테스트.
 *
 * <p>MySQL 실제 컨테이너 기반으로 QnA 조회 API를 검증합니다.
 * H2 기반 QnaQueryE2ETest의 Testcontainers 전환 버전입니다.
 *
 * <p>테스트 대상:
 * <ul>
 *   <li>Q1~Q5: GET /qnas - QnA 목록 조회 (상태 필터, 페이징)</li>
 *   <li>Q6~Q10: GET /qnas/{qnaId} - QnA 상세 조회</li>
 * </ul>
 */
@Tag("e2e")
@Tag("container")
@Tag("qna")
@Tag("query")
@DisplayName("QnA Query Container E2E 테스트")
class QnaContainerQueryE2ETest extends ContainerE2ETestBase {
    ...
    @Nested
    @DisplayName("Q1: GET /qnas - PENDING 필터")
    class SearchQnasByPendingStatusTest {

        @Test
        @Tag("P0")
        @DisplayName("[Q1] PENDING 2건, ANSWERED 1건 존재 시 PENDING 필터 → 2건 반환")
    ...
    @Nested
    @DisplayName("Q2: GET /qnas - ANSWERED 필터")
    class SearchQnasByAnsweredStatusTest {

        @Test
        @Tag("P0")
        @DisplayName("[Q2] ANSWERED 1건, PENDING 2건 존재 시 ANSWERED 필터 → 1건 반환")
    ...
    @Nested
    @DisplayName("Q3: GET /qnas - 상태 필터 없이 전체 조회")
    ...
    @Nested
    @DisplayName("Q5: GET /qnas - 페이징 (size=1)")
    ...
    @Nested
    @DisplayName("Q6: GET /qnas/{qnaId} - PENDING 상세 조회 (답변 없음)")
    ...
    @Nested
    @DisplayName("Q7: GET /qnas/{qnaId} - ANSWERED 상세 조회 (답변 포함)")
    class GetQnaDetailAnsweredTest {

        @Test
        @Tag("P0")
        @DisplayName("[Q7] ANSWERED QnA + reply 1건 → 상세 조회에서 replies 포함 반환")
    ...
    @Nested
    @DisplayName("Q8: GET /qnas/{qnaId} - 존재하지 않는 ID → 404")
    ...
    @Nested
    @DisplayName("Q9: GET /qnas/{qnaId} - replies 대댓글 포함")
    class GetQnaDetailWithNestedRepliesTest {

        @Test
        @Tag("P1")
        @DisplayName("[Q9] ANSWERED QnA + 판매자 답변 + 구매자 추가 질문 → replies 2건 반환")
