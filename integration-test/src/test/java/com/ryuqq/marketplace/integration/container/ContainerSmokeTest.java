...
class ContainerSmokeTest extends ContainerE2ETestBase {
    ...
    void mysqlContainer_IsRunning() {
        assertThat(TestContainersConfig.MYSQL.isRunning())
                .as("MySQL 컨테이너가 실행 중이어야 합니다.")
                .isTrue();
    }
    ...
    void redisContainer_IsRunning() {
        assertThat(TestContainersConfig.REDIS.isRunning())
                .as("Redis 컨테이너가 실행 중이어야 합니다.")
                .isTrue();
    }
