...
import com.ryuqq.marketplace.domain.selleradmin.vo.SellerAdminEmailOutboxIdempotencyKey;
import com.ryuqq.marketplace.domain.selleradmin.vo.SellerAdminEmailOutboxStatus;
import java.time.Instant;

/**
 * 셀러 관리자 이메일 Outbox Aggregate.
 *
 * <p>초대 이메일 발송을 위한 Outbox 패턴 구현체입니다.
 *
 * <p>인증 완료 시 생성되며, 스케줄러에 의해 1분 주기로 PENDING 상태가 처리되어 AWS SES로 초대 메일을 발송합니다.
 *
 ...
 *   <li>idempotencyKey: 이메일 발송 멱등성 보장
 * </ul>
 */
public class SellerAdminEmailOutbox {

    private static final int DEFAULT_MAX_RETRY = 3;

    private final SellerAdminEmailOutboxId id;
    private final SellerId sellerId;
    private final String payload;
    private SellerAdminEmailOutboxStatus status;
    ...
    private final SellerAdminEmailOutboxIdempotencyKey idempotencyKey;

    private SellerAdminEmailOutbox(
            SellerAdminEmailOutboxId id,
            SellerId sellerId,
            String payload,
            ...
            String errorMessage,
            long version,
            SellerAdminEmailOutboxIdempotencyKey idempotencyKey) {
        this.id = id;
     ...
     * @param now 현재 시각
     * @return 새 SellerAdminEmailOutbox 인스턴스
     */
    public static SellerAdminEmailOutbox forNew(SellerId sellerId, String payload, Instant now) {
        SellerAdminEmailOutboxIdempotencyKey idempotencyKey =
                SellerAdminEmailOutboxIdempotencyKey.generate(sellerId, now);
        return new SellerAdminEmailOutbox(
                SellerAdminEmailOutboxId.forNew(),
                sellerId,
                payload,
     ...
     * @return 재구성된 SellerAdminEmailOutbox 인스턴스
     */
    public static SellerAdminEmailOutbox reconstitute(
            SellerAdminEmailOutboxId id,
            SellerId sellerId,
            String payload,
            SellerAdminEmailOutboxStatus status,
        ...
        return new SellerAdminEmailOutbox(
                ...
                errorMessage,
                version,
                SellerAdminEmailOutboxIdempotencyKey.of(idempotencyKey));
    }
    ...
    public void startProcessing(Instant now) {
        ...
        this.status = SellerAdminEmailOutboxStatus.PROCESSING;
        this.updatedAt = now;
    }
    ...
    public void complete(Instant now) {
        this.status = SellerAdminEmailOutboxStatus.COMPLETED;
    ...
    public void failAndRetry(String errorMessage, Instant now) {
        ...
        if (this.retryCount >= this.maxRetry) {
            this.status = SellerAdminEmailOutboxStatus.FAILED;
            this.processedAt = now;
        } else {
            this.status = SellerAdminEmailOutboxStatus.PENDING;
        }
    }
    ...
    public void recoverFromTimeout(Instant now) {
        if (this.status != SellerAdminEmailOutboxStatus.PROCESSING) {
    ...
    // Getters
    public SellerAdminEmailOutboxId id() {
        return id;
    }
    ...
    public SellerAdminEmailOutboxStatus status() {
        return status;
    }
