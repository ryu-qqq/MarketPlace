package com.ryuqq.marketplace.domain.sample.aggregate;

import com.ryuqq.marketplace.domain.common.model.Entity;
import com.ryuqq.marketplace.domain.sample.vo.SampleMoney;
import com.ryuqq.marketplace.domain.sample.vo.SampleOrderItemId;
import java.util.Objects;

/**
 * Sample OrderItem Entity (예시)
 *
 * <p>Aggregate 내부 Entity 예시입니다. (AggregateRoot 아님)
 *
 * <p><strong>TODO: 실제 프로젝트에 맞게 수정</strong>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p><strong>Aggregate 내부 Entity 규칙:</strong>
 *
 * <ul>
 *   <li>✅ Entity 인터페이스 구현 (AggregateRoot 아님!)
 *   <li>✅ Private 생성자 (Factory Method 사용)
 *   <li>✅ equals/hashCode는 ID 기반
 *   <li>✅ 외부 접근 불가 (package-private 또는 public이지만 Aggregate Root를 통해서만)
 *   <li>✅ Repository 없음
 * </ul>
 *
 * @author ryu-qqq
...
public class SampleOrderItem implements Entity {
     ...
     * Private 생성자
     *
     * <p>Factory Method를 통해서만 생성 가능합니다.
     */
    private SampleOrderItem(SampleOrderItemId id, long productId, int quantity, SampleMoney price) {
        this.id = id;
     ...
     *
     * <p>테스트나 알려진 ID로 OrderItem을 생성할 때 사용합니다.
     */
    public static SampleOrderItem of(
            SampleOrderItemId id, long productId, int quantity, SampleMoney price) {
        validateQuantity(quantity);
