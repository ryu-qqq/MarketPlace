...
import com.ryuqq.marketplace.domain.cancel.outbox.vo.CancelOutboxIdempotencyKey;
...
/**
 * 취소 상태 변경 Outbox Aggregate.
 *
 * <p>취소 상태 변경 시 외부 판매채널에 동기화하기 위한 Outbox 패턴 구현체입니다.
 */
public class CancelOutbox {

    private static final int DEFAULT_MAX_RETRY = 3;

    private final CancelOutboxId id;
    private final OrderItemId orderItemId;
    private final CancelOutboxType outboxType;
    private CancelOutboxStatus status;
    ...
    private final CancelOutboxIdempotencyKey idempotencyKey;

    private CancelOutbox(
            CancelOutboxId id,
            OrderItemId orderItemId,
            CancelOutboxType outboxType,
            CancelOutboxStatus status,
            String payload,
            int retryCount,
            int maxRetry,
            Instant createdAt,
    ...
    public static CancelOutbox reconstitute(
            CancelOutboxId id,
            OrderItemId orderItemId,
            CancelOutboxType outboxType,
            CancelOutboxStatus status,
        ...
        return new CancelOutbox(
                ...
                errorMessage,
                version,
                CancelOutboxIdempotencyKey.of(idempotencyKey));
    }
    ...
    public CancelOutboxType outboxType() {
        return outboxType;
    }

    public CancelOutboxStatus status() {
        return status;
    }
    ...
    public CancelOutboxIdempotencyKey idempotencyKey() {
