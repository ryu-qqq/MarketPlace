...
terraform {
  ...
  backend "s3" {
    ...
    key            = "marketplace/ecs-worker/terraform.tfstate"
...
variable "image_tag" {
  ...
  type        = string
  default     = "worker-1-initial" # Fallback only - GitHub Actions will override this

  validation {
    condition     = can(regex("^worker-[0-9]+-[a-zA-Z0-9]+$", var.image_tag))
    error_message = "Image tag must follow format: worker-{build-number}-{git-sha} (e.g., worker-1-abc1234)"
  }
}
