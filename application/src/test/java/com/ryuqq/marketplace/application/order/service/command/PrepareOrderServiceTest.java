...
@DisplayName("PrepareOrderService 단위 테스트")
class PrepareOrderServiceTest {

    @InjectMocks private PrepareOrderService sut;
    ...
    @Nested
    @DisplayName("execute() - 주문상품 발주확인(준비) 처리")
    class ExecuteTest {

        @Test
        @DisplayName("유효한 커맨드로 Shipment를 PREPARING 상태로 전환하고 저장한다")
        void execute_ValidCommand_PreparesShipmentsAndPersists() {
            // given
        ...
        @Test
        @DisplayName("prepare 호출 후 Shipment 상태가 PREPARING으로 변경된다")
        void execute_ValidCommand_ShipmentStatusBecomesPreparing() {
            // given
            OrderItemStatusCommand command = OrderCommandFixtures.orderItemStatusCommand();
            Shipment shipment = ShipmentFixtures.readyShipment();
            Instant now = CommonVoFixtures.now();
        ...
        @Test
        @DisplayName("대상 Shipment가 없으면 persistAll이 빈 목록으로 호출된다")
        void execute_EmptyShipments_CallsPersistAllWithEmptyList() {
            // given
            OrderItemStatusCommand command = OrderCommandFixtures.orderItemStatusCommand();
            Instant now = CommonVoFixtures.now();
            List<OrderItemId> orderItemIds =
