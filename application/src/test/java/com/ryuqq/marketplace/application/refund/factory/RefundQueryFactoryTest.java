...
@DisplayName("RefundQueryFactory 단위 테스트")
class RefundQueryFactoryTest {

    @InjectMocks private RefundQueryFactory sut;
    ...
    class CreateCriteriaTest {
        ...
        @Test
        @DisplayName("검색 파라미터를 RefundSearchCriteria로 변환한다")
        void createCriteria_ValidParams_ReturnsRefundSearchCriteria() {
            // given
            ...
            // when
            RefundSearchCriteria criteria = sut.createCriteria(params);

            // then
            assertThat(criteria).isNotNull();
            assertThat(criteria.queryContext().sortKey()).isEqualTo(RefundSortKey.CREATED_AT);
        ...
        void createCriteria_WithStatusFilter_IncludesStatuses() {
            // given
            RefundSearchParams params = RefundQueryFixtures.searchParamsByStatus("REQUESTED");
            PageRequest pageRequest = PageRequest.of(0, 20);
        ...
        void createCriteria_WithDateRange_CreatesDateRange() {
            // given
            RefundSearchParams params =
                    RefundQueryFixtures.searchParamsByDateRange("2024-01-01", "2024-12-31");
            PageRequest pageRequest = PageRequest.of(0, 20);
