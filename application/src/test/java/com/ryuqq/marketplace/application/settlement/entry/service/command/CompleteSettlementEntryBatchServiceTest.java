...
@DisplayName("CompleteSettlementEntryBatchService 단위 테스트")
class CompleteSettlementEntryBatchServiceTest {

    @InjectMocks private CompleteSettlementEntryBatchService sut;
    ...
    @Nested
    @DisplayName("execute() - 정산 원장 일괄 완료(CONFIRMED) 처리")
    class ExecuteTest {

        @Test
        @DisplayName("entryIds에 해당하는 Entry들을 CONFIRMED로 변환하고 저장한다")
        void execute_ValidCommand_ConfirmsEntriesAndPersistsAll() {
            // given
            CompleteSettlementEntryBatchCommand command =
                    SettlementEntryCommandFixtures.completeSettlementEntryBatchCommand();
        ...
        @Test
        @DisplayName("단건 entryId도 정상 처리한다")
        void execute_SingleEntryId_ConfirmsAndPersists() {
            // given
            CompleteSettlementEntryBatchCommand command =
                    SettlementEntryCommandFixtures.completeSettlementEntryBatchCommand(
                            List.of("entry-single-001"));
            Instant now = Instant.now();
