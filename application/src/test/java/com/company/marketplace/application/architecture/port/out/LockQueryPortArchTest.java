package com.company.marketplace.application.architecture.port.out;
 ...
 *
 * <p>Lock을 사용하는 QueryPort는 정확히 이 규칙을 따라야 합니다:</p>
 * <ul>
 *   <li>인터페이스명: *LockQueryPort (필수)</li>
 *   <li>패키지: ..application..port.out.query.. (필수)</li>
 *   <li>메서드가 있다면 네이밍 패턴 준수 (필수):
 *     <ul>
 *       <li>ForUpdate - Pessimistic Write Lock (SELECT ... FOR UPDATE)</li>
 *       <li>ForShare - Pessimistic Read Lock (SELECT ... FOR SHARE)</li>
 *       <li>WithLock - 일반 Lock (Pessimistic Write)</li>
 *       <li>WithReadLock - 읽기 Lock</li>
 *       <li>Nowait - 대기 없이 실패 (FOR UPDATE NOWAIT)</li>
 *       <li>SkipLocked - 락 걸린 행 건너뛰기 (FOR UPDATE SKIP LOCKED)</li>
 *       <li>WithOptimisticLock - Optimistic Lock (버전 기반)</li>
 *     </ul>
 *   </li>
 *   <li>반환: Optional<Domain> (단건만, 필수)</li>
 *   <li>특정 메서드 필수 여부: 없음 (프로젝트에 맞게 선택적 구현)</li>
 *   <li>주의사항: @Transactional 내에서만 호출 (Lock은 Transaction 내에서만 유효)</li>
 * </ul>
 *
 * <h3>사용 예시:</h3>
 * <pre>
...
class LockQueryPortArchTest {
    ...
    void lockQueryPort_MethodsMustFollowLockNamingPattern() {
        ArchRule rule = methods()
            .that().areDeclaredInClassesThat().haveSimpleNameEndingWith("LockQueryPort")
            .and().arePublic()
            .should().haveNameMatching(".*(ForUpdate|ForShare|WithLock|WithReadLock|Nowait|SkipLocked|WithOptimisticLock)")
    ...
    void lockQueryPort_MustNotReturnListOrPage() {
        ArchRule rule = noMethods()
            ...
            .orShould().haveRawReturnType(".*PageResponse")
