...
/**
 * 교환 도메인 정산 역분개 Entry 생성 프로세서.
 *
 * <p>교환 완료 시 역분개 Entry를 생성합니다. UseCase를 거치지 않고 Factory + PersistenceFacade를 직접 사용합니다.
 */
@Component
public class ExchangeSettlementProcessor {

    private static final Logger log = LoggerFactory.getLogger(ExchangeSettlementProcessor.class);

    private final SettlementEntryCommandFactory factory;
    private final SettlementEntryPersistenceFacade persistenceFacade;

    public ExchangeSettlementProcessor(
    ...
    /** 교환 역분개 Entry를 생성하고 저장한다. 실패해도 클레임 처리를 막지 않는다. */
    public void createReversalEntry(String orderItemId, long sellerId, String exchangeClaimId) {
        try {
            CreateReversalEntryCommand command =
                    new CreateReversalEntryCommand(
                            orderItemId, sellerId, exchangeClaimId, "EXCHANGE", 0, 0);
