...
/**
 * 레거시 주문 벌크 시딩 분산 락 Manager.
 *
 * <p>ECS 다중 인스턴스 환경에서 Seeder 스케줄러가 동시 실행되는 것을 방지합니다. Seeder는 Read → 없으면 INSERT 패턴이라 CAS(낙관적 락)로 보호할
 * 수 없어 스케줄러 실행 단위로 분산 락을 겁니다.
 */
@Component
public class LegacyOrderSeederLockManager {

    private static final Logger log = LoggerFactory.getLogger(LegacyOrderSeederLockManager.class);
    ...
    public LegacyOrderSeederLockManager(DistributedLockPort lockPort) {
    ...
    public <T> T executeWithLock(Supplier<T> action, T fallback) {
        ...
        if (!acquired) {
            log.warn("레거시 주문 시딩 분산 락 획득 실패 - 다른 인스턴스가 실행 중. 이번 주기 스킵");
            return fallback;
        }
    ...
    private record SeederLockKey() implements LockKey {
        private static final String KEY = "lock:legacy-order-conversion:seeder";
