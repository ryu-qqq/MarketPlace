...
/**
 * 환불 아웃박스 단건 Relay 처리기.
 *
 * <p>PENDING → startProcessing → SQS 발행 → complete / fail
 */
@Component
@ConditionalOnProperty(prefix = "sqs.queues", name = "refund-outbox")
public class RefundOutboxRelayProcessor {

    private static final Logger log = LoggerFactory.getLogger(RefundOutboxRelayProcessor.class);
    ...
    public boolean relay(RefundOutbox outbox) {
        ...
        try {
            ...
            log.info(
                    "환불 Outbox SQS 발행 성공: outboxId={}, orderItemId={}",
                    outbox.idValue(),
            ...
            log.error(
                    "환불 Outbox Relay 실패: outboxId={}, orderItemId={}, error={}",
                    outbox.idValue(),
                    outbox.orderItemIdValue(),
                    e.getMessage(),
                    e);
    ...
    private String buildMessageBody(RefundOutbox outbox) {
        try {
            ...
            throw new RuntimeException("환불 Outbox SQS 메시지 직렬화 실패", e);
        }
    }
}
