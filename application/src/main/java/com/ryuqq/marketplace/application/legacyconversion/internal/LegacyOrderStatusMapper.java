...
public class LegacyOrderStatusMapper {
    ...
    public OrderStatusResolution resolve(String legacyStatus) {
        return switch (legacyStatus) {
            ...
            case "ORDER_PROCESSING" ->
                    OrderStatusResolution.normalOrder(null);
            case "DELIVERY_PENDING" ->
                    OrderStatusResolution.normalOrder(ShipmentStatus.READY);
            ...
            case "SALE_CANCELLED", "SALE_CANCELLED_COMPLETED", "CANCEL_REQUEST_COMPLETED" ->
                    OrderStatusResolution.withCancel(ShipmentStatus.DELIVERED, CancelStatus.COMPLETED);
            case "CANCEL_REQUEST_CONFIRMED" ->
                    OrderStatusResolution.withCancel(ShipmentStatus.DELIVERED, CancelStatus.APPROVED);

            // 반품 (배송 완료 후 반품이므로 Shipment DELIVERED)
            case "RETURN_REQUEST" ->
                    OrderStatusResolution.withRefund(ShipmentStatus.DELIVERED, RefundStatus.REQUESTED);
            case "RETURN_REQUEST_COMPLETED" ->
                    OrderStatusResolution.withRefund(ShipmentStatus.DELIVERED, RefundStatus.COMPLETED);
            case "RETURN_REQUEST_REJECTED" ->
                    OrderStatusResolution.withRefund(ShipmentStatus.DELIVERED, RefundStatus.REJECTED);
            case "RETURN_REQUEST_CONFIRMED" ->
                    OrderStatusResolution.withRefund(ShipmentStatus.DELIVERED, RefundStatus.COLLECTING);
    ...
    public record OrderStatusResolution(
        ...
        static OrderStatusResolution withCancel(ShipmentStatus shipmentStatus, CancelStatus cancelStatus) {
            return new OrderStatusResolution(shipmentStatus, ClaimType.CANCEL, cancelStatus, null);
        }

        static OrderStatusResolution withRefund(ShipmentStatus shipmentStatus, RefundStatus refundStatus) {
