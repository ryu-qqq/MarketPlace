...
public class LegacyNoticeCategoryResolver {
    ...
    /** categoryId 기반 NoticeCategory 조회 (최초 등록 시 사용). */
    public NoticeCategory resolveByCategoryId(long categoryId) {
        Category category = categoryReadManager.getById(CategoryId.of(categoryId));
        return resolveByCategoryGroup(category.categoryGroup());
    }
