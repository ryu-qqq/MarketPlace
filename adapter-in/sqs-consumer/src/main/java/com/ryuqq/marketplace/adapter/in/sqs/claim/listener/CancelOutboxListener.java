...
import com.ryuqq.marketplace.adapter.in.sqs.claim.dto.CancelOutboxSqsMessage;
...
/**
 * 취소 Outbox SQS 컨슈머 리스너.
 *
 * <p>SQS 메시지를 수신하여 취소 Outbox 처리를 실행합니다.
 */
@Component
@ConditionalOnProperty(
        prefix = "sqs.consumer.cancel-outbox",
        name = "enabled",
        havingValue = "true",
        matchIfMissing = false)
public class CancelOutboxListener {

    private static final Logger log = LoggerFactory.getLogger(CancelOutboxListener.class);

    private final ExecuteCancelOutboxUseCase useCase;
    ...
    @SqsListener("${sqs.queues.cancel-outbox}")
    public void onMessage(CancelOutboxSqsMessage message) {
        ...
        try {
            ...
            log.error(
                    "Cancel Outbox 메시지 처리 실패: outboxId={}, error={}",
                    message.outboxId(),
                    e.getMessage(),
                    e);
            throw e;
