package com.company.marketplace.adapter.in.rest.architecture;
 ...
 *
 * <p>REST API Layer 전체 아키텍처 및 레이어 분리를 검증합니다.</p>
 *
 * <p><strong>검증 규칙:</strong></p>
 * <ul>
 *   <li>규칙 1: Package 구조 검증 (controller, dto, mapper, error)</li>
 *   <li>규칙 2: Bounded Context별 패키지 구조 검증</li>
 *   <li>규칙 3: Common 패키지 구조 검증</li>
 *   <li>규칙 4: DTO 패키지 분리 (command/query/response)</li>
 *   <li>규칙 5: REST API Layer는 Application Layer Port 의존 필수</li>
 *   <li>규칙 6: REST API Layer는 Domain Layer 직접 의존 금지</li>
 *   <li>규칙 7: ErrorMapper만 Domain Exception 의존 허용</li>
 *   <li>규칙 8: Controller는 Domain 객체 반환 금지</li>
 *   <li>규칙 9: Mapper는 Domain 직접 의존 금지 (ErrorMapper 제외)</li>
 *   <li>규칙 10: REST API Layer는 Persistence Layer 의존 금지</li>
 *   <li>규칙 11: Config 클래스는 config 패키지에 위치</li>
 *   <li>규칙 12: Properties 클래스는 config/properties 패키지에 위치</li>
 * </ul>
 *
 * <p><strong>참고 문서:</strong></p>
 * <ul>
 *   <li>rest-api-guide.md - REST API Layer 전체 가이드</li>
 * </ul>
 *
...
class RestApiLayerArchTest {
    ...
    void restApiLayer_MustHaveStandardPackageStructure() {
        ArchRule rule = classes()
            ...
            .and().resideOutsideOfPackages("..architecture..", "..common..")
            .and().haveSimpleNameNotEndingWith("Test")
            .and().haveSimpleNameNotEndingWith("TestSupport")
            ...
            .because("REST API Layer는 controller, dto, mapper, error, config, filter 패키지로 구성되어야 합니다");
    ...
    void boundedContext_ShouldHaveStandardSubPackages() {
        ...
        ArchRule rule = classes()
            .that().resideInAPackage("..adapter.in.rest..")
            .and().resideOutsideOfPackages("..common..", "..config..", "..architecture..")
            .should().resideInAnyPackage(
                "..controller..",
                "..dto..",
                "..mapper..",
    ...
    void commonPackage_MustHaveStandardStructure() {
        ArchRule rule = classes()
            .that().resideInAPackage("..adapter.in.rest.common..")
    ...
    void dto_MustBeSeparatedIntoCommandQueryResponse() {
        ArchRule rule = classes()
            ...
            .and().resideOutsideOfPackages("..common.dto..", "..architecture..")
    ...
    void controller_MustDependOnApplicationPorts() {
        ArchRule rule = classes()
            ...
            .and().haveSimpleNameNotContaining("GlobalExceptionHandler")
    ...
    void restApiLayer_MustNotDependOnDomain() {
        ArchRule rule = noClasses()
            .that().resideInAPackage("..adapter.in.rest..")
            .and().resideOutsideOfPackages("..error..", "..architecture..")
            .and().haveSimpleNameNotContaining("GlobalExceptionHandler")
            .and().haveSimpleNameNotEndingWith("ErrorMapper")
            .should().dependOnClassesThat().resideInAnyPackage("com.ryuqq.domain..")
            .because("REST API Layer는 Domain Layer를 직접 의존하면 안 됩니다 (GlobalExceptionHandler, ErrorMapper 제외)");
    ...
    void globalExceptionHandler_MustDependOnDomainException() {
        ArchRule rule = classes()
            ...
            .should().dependOnClassesThat().resideInAPackage("com.ryuqq.domain..exception..")
    ...
    void mapper_MustNotDependOnDomain() {
        ArchRule rule = noClasses()
            ...
            .should().dependOnClassesThat().resideInAnyPackage("com.ryuqq.domain..")
            .because("Mapper는 Application DTO만 사용하며 Domain 직접 의존은 금지됩니다 (ErrorMapper 제외)");
    ...
    void restApiLayer_MustNotDependOnPersistence() {
        ArchRule rule = noClasses()
            ...
            .because("REST API Layer는 Persistence Layer를 직접 의존하면 안 되며 Application Layer를 통해야 합니다");
