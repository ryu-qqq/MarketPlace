package com.company.marketplace.adapter.in.rest.architecture.common;
 ...
 *
 * <p>Common API Response 패턴을 검증합니다.</p>
 *
 * <p><strong>검증 규칙:</strong></p>
 * <ul>
 *   <li>규칙 1: Common Response DTOs는 common.dto 패키지에 위치</li>
 *   <li>규칙 2: Common Response DTOs는 Record 타입이어야 함</li>
 *   <li>규칙 3: ApiResponse는 static factory method 패턴 사용</li>
 *   <li>규칙 4: PageApiResponse는 from() 메서드 필수</li>
 *   <li>규칙 5: ErrorInfo는 validation 필수</li>
 *   <li>규칙 6: Common Response DTOs는 Lombok 금지</li>
 *   <li>규칙 7: Common Response DTOs는 public이어야 함</li>
 *   <li>규칙 8: Common Response DTOs는 final이어야 함 (Record 특성)</li>
 * </ul>
 *
 * <p><strong>참고 문서:</strong></p>
 * <ul>
 *   <li>rest-api-guide.md - REST API Layer 전체 가이드</li>
 *   <li>dto/response/response-dto-guide.md - Response DTO 가이드</li>
 * </ul>
 *
...
class ApiResponseArchTest {
    ...
    void commonResponseDtos_MustBeInCommonDtoPackage() {
        ArchRule rule = classes()
            .that().haveNameMatching(".*ApiResponse|.*ErrorInfo")
            .and().resideInAPackage("..adapter.in.rest..")
            .and().areNotNestedClasses()
            .should().resideInAPackage("..common.dto..")
            .because("Common Response DTOs(ApiResponse, PageApiResponse, SliceApiResponse, ErrorInfo)는 common.dto 패키지에 위치해야 합니다");
    ...
    void commonResponseDtos_MustBeRecords() {
        ArchRule rule = classes()
            .that().haveNameMatching(".*ApiResponse|.*ErrorInfo")
            .and().resideInAPackage("..common.dto..")
            .and().areNotNestedClasses()
            .should().beRecords()
    ...
    void apiResponse_MustHaveStaticFactoryMethods() {
        ArchRule successRule = methods()
            .that().areDeclaredInClassesThat().haveSimpleName("ApiResponse")
            .and().haveName("ofSuccess")
            .should().bePublic()
            .andShould().beStatic()
            .because("ApiResponse는 ofSuccess() static factory method가 필수입니다");

        ArchRule failureRule = methods()
            .that().areDeclaredInClassesThat().haveSimpleName("ApiResponse")
            .and().haveName("ofFailure")
            .should().bePublic()
            .andShould().beStatic()
    ...
    void pageApiResponse_MustHaveFromMethod() {
        ArchRule rule = methods()
            .that().areDeclaredInClassesThat().haveSimpleName("PageApiResponse")
            .and().haveName("from")
            .should().bePublic()
            .andShould().beStatic()
            .because("PageApiResponse는 Application Layer PageResponse를 변환하는 from() static method가 필수입니다");
    ...
    void errorInfo_ShouldHaveValidation() {
        ...
        ArchRule rule = classes()
            .that().haveSimpleName("ErrorInfo")
            .should().beRecords()
            .because("ErrorInfo는 Record로 정의되어 compact constructor에서 validation을 수행해야 합니다");
