...
public record SellerCancelBatchApiRequest(
    ...
    public record SellerCancelItemApiRequest(
            @Schema(description = "주문 상품 ID (UUIDv7)", example = "01940001-0000-7000-8000-000000000001", requiredMode = Schema.RequiredMode.REQUIRED)
                    @NotBlank
                    String orderId,
            @Schema(description = "취소 수량", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
                    @Positive
                    int cancelQty,
            @Schema(description = "취소 사유 유형", example = "OUT_OF_STOCK", requiredMode = Schema.RequiredMode.REQUIRED)
                    @NotBlank
                    String reasonType,
            @Schema(description = "취소 상세 사유", example = "재고 소진")
                    String reasonDetail) {}
}
