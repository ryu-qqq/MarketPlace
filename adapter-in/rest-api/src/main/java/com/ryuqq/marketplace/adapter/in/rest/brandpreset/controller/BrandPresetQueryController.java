...
public class BrandPresetQueryController {
    ...
    private final GetBrandPresetDetailUseCase getBrandPresetDetailUseCase;
    ...
    public BrandPresetQueryController(
            ...
            GetBrandPresetDetailUseCase getBrandPresetDetailUseCase,
            BrandPresetQueryApiMapper mapper) {
        this.searchBrandPresetByOffsetUseCase = searchBrandPresetByOffsetUseCase;
        this.getBrandPresetDetailUseCase = getBrandPresetDetailUseCase;
        this.mapper = mapper;
    }
    ...
    @Operation(summary = "브랜드 프리셋 상세 조회", description = "브랜드 프리셋 상세 정보를 조회합니다.")
    @PreAuthorize("@access.authenticated()")
    @RequirePermission(value = "brand-preset:read", description = "브랜드 프리셋 조회")
    @GetMapping(BrandPresetAdminEndpoints.BRAND_PRESET_ID)
    public ResponseEntity<ApiResponse<BrandPresetDetailApiResponse>> getBrandPreset(
            @PathVariable(BrandPresetAdminEndpoints.PATH_BRAND_PRESET_ID) Long brandPresetId) {

        BrandPresetDetailResult result = getBrandPresetDetailUseCase.execute(brandPresetId);
