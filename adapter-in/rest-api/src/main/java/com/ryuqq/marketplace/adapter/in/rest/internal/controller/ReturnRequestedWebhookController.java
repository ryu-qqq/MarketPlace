 ...
 * 반품 요청 웹훅 Controller.
 *
 * <p>배송 완료 후 구매자 반품 요청. ClaimSync 파이프라인으로 환불을 생성합니다.
 *
 * @author ryu-qqq
 * @since 1.2.0
 */
...
public class ReturnRequestedWebhookController {
    ...
    @Operation(summary = "반품 요청 웹훅", description = "구매자 반품 요청을 처리합니다.")
    @PostMapping(InternalWebhookEndpoints.RETURN_REQUESTED)
    public ResponseEntity<ApiResponse<ClaimSyncWebhookResponse>> handleReturnRequested(
            @RequestBody @Valid ReturnRequestedWebhookRequest request) {
