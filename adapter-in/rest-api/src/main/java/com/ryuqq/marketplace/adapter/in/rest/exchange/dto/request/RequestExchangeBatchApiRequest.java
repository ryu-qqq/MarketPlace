...
public record RequestExchangeBatchApiRequest(
    ...
    public record ExchangeRequestItemApiRequest(
            ...
            @Schema(
                    ...
                    @NotBlank
                    String reasonType,
            @Schema(description = "교환 상세 사유", example = "사이즈 변경 원합니다") String reasonDetail,
            @Schema(description = "원 상품 ID", requiredMode = Schema.RequiredMode.REQUIRED)
                    long originalProductId,
            @Schema(description = "원 SKU 코드", requiredMode = Schema.RequiredMode.REQUIRED) @NotBlank
            ...
            @Schema(description = "교환 대상 수량", requiredMode = Schema.RequiredMode.REQUIRED) @Positive
                    int targetQuantity) {}
}
