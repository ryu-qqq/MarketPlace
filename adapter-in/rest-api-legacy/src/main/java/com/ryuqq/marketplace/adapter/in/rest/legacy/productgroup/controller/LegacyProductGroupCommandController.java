...
import static com.ryuqq.marketplace.adapter.in.rest.legacy.productgroup.LegacyProductGroupEndpoints.PRICE;
...
public class LegacyProductGroupCommandController {
    ...
    private final ResolveLegacyProductContextUseCase resolveLegacyProductContextUseCase;
    ...
    public LegacyProductGroupCommandController(
            ResolveLegacyProductContextUseCase resolveLegacyProductContextUseCase,
    ...
    public ResponseEntity<LegacyApiResponse<Long>> updateProductGroupFull(
        ...
        legacyProductGroupFullUpdateUseCase.execute(
                legacyProductGroupCommandApiMapper.toUpdateFullCommand(
                        productGroupId, request, context));

        return ResponseEntity.ok(LegacyApiResponse.of(productGroupId));
    }

    @Operation(summary = "레거시 가격 수정", description = "세토프 어드민 호환 상품그룹의 가격을 수정합니다.")
    @PreAuthorize("@legacyAccess.isProductOwnerOrMaster(#productGroupId)")
    @PatchMapping(PRICE)
